...
for idx,cat in enumerate(categories):
    ...
    for i, f in enumerate(files):
        ...
        #되는지 실험
        #이미지 그레이스케일로 불러오기
        gry_face=cv2.imread(f,cv2.IMREAD_GRAYSCALE)
        #trainingData X리스트에 이미지를 바이트 배열로 추가
        X.append(np.asarray(gry_face,dtype=np.uint8))
        #Label 리스트에는 카테고리 이름(사실상 사람이름)을 추가
        Y.append(np.asarray(cat))
X=np.array(X)
Y=np.array(Y)
'''
